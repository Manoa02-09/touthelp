...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Styles existants du site (gardés) */
...
<body class="bg-white">
            ...
            <div class="p-6">
                <div class="prose max-w-none">
                    ...
                    <div class="mt-6 flex flex-wrap gap-4">
                        <a href="{{ route('calendrier') }}" class="bg-blue-700 hover:bg-blue-800 text-white px-6 py-2 rounded-lg" onclick="closeExpertiseModal('inter')">📅 Voir le calendrier des formations</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="prose max-w-none">
                    ...
                    <div class="mt-6 border-l-4 border-green-600 pl-4">
                        ...
                        <p>Nous vous accompagnons dans la mise en place et la structuration de vos systèmes de management.</p>
                        <h4 class="font-semibold mt-2">Ce qu'on vous propose :</h4>
                        <ul class="list-disc pl-6"><li>Mise en place de systèmes (ISO, HSE, SMSST, RSE…)</li><li>Structuration organisationnelle</li><li>Rédaction de processus et procédures</li><li>Élaboration de documents opérationnels</li><li>Appui et suivi jusqu'à la mise en œuvre</li></ul>
                    ...
                    <div class="mt-6 border-l-4 border-blue-600 pl-4">
                        ...
                        <ul class="list-disc pl-6"><li>Audit interne</li><li>Audit à blanc (pré-certification)</li><li>Diagnostic organisationnel</li><li>Évaluation de conformité</li></ul>
                        <div class="bg-blue-50 p-3 rounded-lg mt-2"><p class="font-semibold text-blue-800">Résultat :</p><p class="text-blue-700">Une vision claire de votre situation et des actions concrètes pour progresser.</p></div>
                    </div>
                    <div class="mt-6 p-4 bg-gray-100 rounded-lg text-center"><p><i class="fas fa-sync-alt text-green-600 mr-2"></i> L'accompagnement et l'audit sont complémentaires. Nous pouvons intervenir à différentes étapes selon vos besoins.</p></div>
            ...
            <div class="relative overflow-hidden group">
                <div class="flex overflow-x-auto scrollbar-hide gap-8 py-4 px-2 scroll-smooth" id="partenairesScroll">
                    ...
                    <div class="flex-shrink-0 w-32 h-32 bg-gray-50 rounded-full flex items-center justify-center shadow-md hover:shadow-lg transition">
                        @if($partenaire->logo)<img src="{{ asset('storage/' . $partenaire->logo) }}" alt="{{ $partenaire->nom_entreprise }}" class="max-w-full max-h-full p-2 object-contain">@else<span class="text-gray-400 text-xs">{{ $partenaire->nom_entreprise }}</span>@endif
                    </div>
                    @endforeach
                </div>
                ...
                <div class="flex overflow-x-auto scrollbar-hide gap-6 py-4 px-2 scroll-smooth" id="avisScroll">
                    ...
                    <div class="flex-shrink-0 w-80 bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                        ...
                        <div class="flex items-center gap-3">
                            @if($a->logo_entreprise)<img src="{{ asset('storage/' . $a->logo_entreprise) }}" class="w-10 h-10 rounded-full object-cover">@endif
                            <div><p class="font-bold text-gray-800">{{ $a->entreprise_nom }}</p><p class="text-sm text-gray-500">{{ $a->contact_fonction ?? 'Client' }}</p></div>
                        </div>
                    </div>
                    @endforeach
                </div>
    ...
    <!-- ===== SECTION CONTACT (style rose/rouge de ta copine) ===== -->
            ...
            <div class="flex flex-col lg:flex-row items-center gap-12">
                ...
                <div class="lg:w-1/2 w-full">
                    ...
                    <form id="footerContactForm">
                        @csrf
                        <div class="mb-6">
                            <input type="text" name="nom" id="footer_nom" placeholder="Nom complet" class="w-full px-6 py-4 text-lg rounded-xl bg-white/10 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-4 focus:ring-yellow-400" required maxlength="150">
                        </div>
                        <div class="mb-6">
                            <input type="email" name="email" id="footer_email" placeholder="Email" class="w-full px-6 py-4 text-lg rounded-xl bg-white/10 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-4 focus:ring-yellow-400" required maxlength="150">
                        </div>
                        <div class="mb-6">
                            <input type="text" name="telephone" id="footer_telephone" placeholder="Téléphone" class="w-full px-6 py-4 text-lg rounded-xl bg-white/10 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-4 focus:ring-yellow-400" maxlength="30">
                        </div>
                        <div class="mb-8">
                            <textarea name="message" id="footer_message" rows="4" placeholder="Votre message..." class="w-full px-6 py-4 text-lg rounded-xl bg-white/10 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-4 focus:ring-yellow-400" required maxlength="2000"></textarea>
                        </div>
                        <button type="submit" id="footerSubmitBtn" class="w-full bg-yellow-500 hover:bg-yellow-600 text-rose-800 font-bold py-4 rounded-xl transition text-xl">
                            Envoyer le message
                        </button>
                    </form>
    ...
    function canSendMessage(email) {
        ...
        if (now - last < 5000) { // 5 secondes minimum entre deux messages
            return false;
        }
    ...
    function validateInput(input, type) {
        // Nettoyer les entrées (strip_tags simple)
        ...
        if (type === 'name') {
            // Uniquement lettres, espaces, tirets, accents
    ...
    window.addEventListener('click', (e) => {
        ...
        if (modal.classList.contains('active') && !modal.contains(e.target) && !robot.contains(e.target) && !anyExpertiseOpen) {
            closeChatModal();
        }
    });
    ...
    /* ============================================================
       AFFICHER MESSAGES (avec escape HTML renforcé)
       ============================================================ */
    function renderMessages(messages) {
        ...
        if (!last.reponse_admin || !last.reponse_admin.trim()) {
            html += '<div class="pending-tag">⏳ En attente de réponse...</div>';
        }
        area.innerHTML = html;
        scrollChatToBottom();
    }
    ...
    /* ============================================================
       ENVOYER MESSAGE API (avec protection anti-spam)
       ============================================================ */
    async function sendMessageAPI(nom, email, telephone, message) {
        // Vérification anti-spam
        if (!canSendMessage(email)) {
            return { success: false, message: 'Veuillez patienter avant d\'envoyer un autre message' };
        }
        ...
        if (!validNom || !validEmail || !validMessage) {
    ...
    document.getElementById('footerContactForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                openChatModal();
                await loadMessages();
                document.getElementById('footerContactSuccess').innerHTML = 'Message envoyé avec succès ! Nous vous répondrons rapidement.';
                document.getElementById('footerContactSuccess').classList.remove('hidden');
                setTimeout(() => document.getElementById('footerContactSuccess').classList.add('hidden'), 5000);
            } else {
                document.getElementById('footerContactError').innerHTML = result.message || 'Erreur lors de l\'envoi. Réessayez.';
                document.getElementById('footerContactError').classList.remove('hidden');
                setTimeout(() => document.getElementById('footerContactError').classList.add('hidden'), 5000);
            }
        } catch(e) {
            document.getElementById('footerContactError').innerHTML = 'Erreur réseau. Réessayez.';
            document.getElementById('footerContactError').classList.remove('hidden');
            setTimeout(() => document.getElementById('footerContactError').classList.add('hidden'), 5000);
        }
    ...
    /* ============================================================
       UTILITAIRES (sécurité renforcée)
       ============================================================ */
